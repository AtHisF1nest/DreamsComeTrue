...
@Component({
  selector: 'app-management-dreams-connections',
  templateUrl: './management-dreams-connections.component.html',
  styleUrls: ['./management-dreams-connections.component.css']
})
export class ManagementDreamsConnectionsComponent implements OnInit {
  ...
  loadConnections() {
    this.todosService.getTodosDreamsConnections(this.selectedCategory.id).subscribe(data => {
  ...
  loadNotConnected() {
    this.todosService.getNotConnectedDreams(this.selectedCategory.id).subscribe(data => {
      this.todoList = data;
    }, error => {
      this.alertify.error('Wystąpił problem podczas pobierania danych.');
    });
